...
def get_categorical_columns_train(df_train):
    """
    Obtiene columnas de cada categoría de todas las columnas de tipo category de un dataframe
...
# Aplica un get label a las columnas del dataframe train y test

def apply_label_encoder(df_train, df_test):
    """
    Aplica un get label a las columnas del dataframe train y test.
    ...
    returns:
        tupla.  Primer elemento: Dataframe de entrenamiento con las columnas categorizadas en int64. 
                Segundo elemento: Dataframe de prueba con las columnas categorizadas en int64.
    """

    df_copy_train = df_train.copy()
    df_copy_test = df_test.copy()
    combined_df = pd.concat([df_train, df_test], axis=0)
    for col in df_copy_train.select_dtypes(["category"]).columns:
        encoder = LabelEncoder()
        encoder.fit(combined_df[col].astype(str))
        df_copy_train[col] = encoder.transform(df_copy_train[col])
        df_copy_test[col] = encoder.transform(df_copy_test[col])
    output = (df_copy_train, df_copy_test)
    return output
